...
{
    public class GhostlyColumnListVisualizer : IListVisualizer
    {
        ...
        public GhostlyColumnListVisualizer(int minColumnSize, int minSpacerSize, float columnProportion)
        {
        ...
        public int Redraw(WriteableBitmap writeableBitmap, SortState<int> sortState, ColorSet colorSet)
        {
            ...
            for (int i = 0; i < size; i++)
            {
                ...
                if (scaledValue > 0)
                {
                    int ghostlyValue = Math.Max(scaledValue - ColumnSize, 0);
                    writeableBitmap.FillRectangle(xCurrent, yOrigin - scaledValue, xCurrent + ColumnSize, yOrigin - ghostlyValue, currentColor);
                    writeableBitmap.FillRectangle(xCurrent, yOrigin - ghostlyValue, xCurrent + ColumnSize, yOrigin, ghostlyColor);
                }
                else if (scaledValue < 0)
                {
                    int ghostlyValue = Math.Min(scaledValue + ColumnSize, 0);
                    writeableBitmap.FillRectangle(xCurrent, yOrigin - ghostlyValue, xCurrent + ColumnSize, yOrigin - scaledValue, currentColor);
                    writeableBitmap.FillRectangle(xCurrent, yOrigin, xCurrent + ColumnSize, yOrigin - ghostlyValue, ghostlyColor);
                }
